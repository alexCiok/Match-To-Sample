...
trial {
	trial_duration = 2655; #16.66ms/s delay ~45ms 2700; #BC: Should link to the randomized delay time
	picture {
		text { caption = "+"; };
		x= 0; y = 0;
	};
...
trial {
	...
	trial_duration =  3933; #16.66ms/s delay ~67ms #4000; # SET THE DURATION OF TEST TRIAL 
	terminator_button = 1,2; 
...
trial {
	trial_duration = 17700; #16.66ms/s delay ~300ms #18000; #6TRs for Dummies (12s), 3 for Dr. Song (6s)
 	picture {
		text { caption = "+"; };
		x= 0; y = 0;
	};
...
trial {
	trial_duration = 13767; #16.66ms/s delay ~233ms #14000;
	picture {
		text { caption = "+"; };
		x= 0; y = 0;
	};
...
if cols == 3 then 
	study_data.open("study_3.txt");
	distract_data.open("distr_3.txt");
end;
if cols == 5 then
	study_data.open("study_5.txt");
	distract_data.open("distr_5.txt");
end;

int count1 = 0;
int array_fill_count = 1;
array <int> int_study[30][0];
array<int> int_distr[30][0]; #AC
int val;
int distr;
loop until 
	count1 == 30
begin
	...
	begin
		...
		distr = distract_data.get_int(); #AC
		val = study_data.get_int();
		int_study[count1].add(val);
		int_distr[count1].add(distr); #AC
	end;
end;
...
begin
	...
	sequence[2].assign(int_distr[rand_idx]); #sequence[2].assign(sequence[1]);	
